defmodule Calculator do
  ...
  """
  def start() do
    joao = %Student{name: "João Victor", results: build_subjects([5.2, 3.9, 2.7, 9.8])}
    maria = %Student{name: "Maria do Socorro", results: build_subjects([8.4, 7.5, 9.4, 2.8])}
    adam = %Student{name: "Adam Andrade", results: build_subjects([1.3, 8.5, 8.9, 7.6])}
    cris = %Student{name: "Cristiano da Silva", results: build_subjects([5.4, 4.9, 2.2, 3.8])}

    result = calculate([joao, maria, adam, cris])
  ...
  defp build_subjects(results) do
    subjects = ["Math", "Portuguese", "Geography", "History"]
